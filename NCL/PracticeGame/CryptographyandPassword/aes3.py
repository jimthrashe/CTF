...
def run_decryption(timestamp, output_file, success_flag):
    try:
        decryption_password = timestamp  # Use the current timestamp as the decryption password
        openssl_command = f'openssl enc -d -aes-256-cbc -in ciphertext -out "{output_file}" -pass pass:{decryption_password}'
